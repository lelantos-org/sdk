...
describe("hashStringStruct", () => {
    it("is 32 bytes", () => {
        expect(hashStringStruct(DOMAIN, ["Lelantos", "1"])).toHaveLength(32);
    });

    it("depends on every value", () => {
        const base = hashStringStruct(DOMAIN, ["Lelantos", "1"]);
        expect(hashStringStruct(DOMAIN, ["Lelantos", "3"])).not.toEqual(base);
        expect(hashStringStruct(DOMAIN, ["Lelantos!", "1"])).not.toEqual(base);
    });
...
describe("typedDataDigest", () => {
    const a = hashStringStruct(DOMAIN, ["Lelantos", "1"]);
    const b = hashStringStruct("Msg(string purpose,string version)", ["nsk-derivation", "1"]);
