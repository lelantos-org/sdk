...
// `installNodeRayonWorker` in `wasm-prover.ts`. Implements the
...
//
// Stays as a static .mjs asset (copied to `dist/wasm/` by the build) so the
...
const dbg = (m) => {
    if (process.env.LELANTOS_RAYON_DEBUG)
        console.error(`[rayon-worker ${threadId}]`, m);
};
