...
describe("bigintFrom", () => {
    ...
    it("rejects the shapes BigInt() would otherwise accept or crash on", () => {
        // BigInt("") is 0n and BigInt(" 1 ") is 1n.
...
describe("hexInt", () => {
    ...
    it("reads an all-digit bare-hex value as hex, not decimal", () => {
        // The server sends `commitmentHex` and `cmHex` without a `0x`, and a
        // value made only of decimal digits is also a valid decimal string, so
        // `bigintFrom` would silently decode 1234 instead of 4660.
