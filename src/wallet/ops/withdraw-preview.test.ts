...
/**
 * An asset with no ladder. Ladders are derived from `scale` and `decimals`, so the only way to
 * have none is to opt out.
 */
...
describe("previewWithdraw", () => {
    ...
    it("flags an off-ladder amount and points at the nearest denomination", () => {
        // Not rejected, but it publishes a near-unique integer, so it is flagged.
    ...
    it("grows the net with the yield index, the denomination unchanged", () => {
        // Fee-free on both sides, so only the index differs.
        ...
        const flat = previewWithdraw({ amount: circuitAmount(1_000_000_000n), asset: free });
        const grown = previewWithdraw({ amount: circuitAmount(1_000_000_000n), asset: yielding });
...
describe("opting out of the ladder", () => {
    // `WalletConfig.denominations: false` gives every AssetInfo an empty ladder, which all
    // downstream paths read.
