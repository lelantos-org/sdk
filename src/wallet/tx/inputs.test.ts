...
// The spend-time blinder must be fresh per transaction: `in_cv = value·gen + rcv·H` is a public
// input, so a constant `rcv` exposes an unblinded commitment to the amount and makes two spends
// of equal value identical.
...
describe("buildInputSlots", () => {
    ...
    /// A cross-asset fee puts notes of two assets in one spend. Each slot must
    /// carry its note's stored asset; overwriting it would change the commitment
    /// whose Merkle membership is proven.
