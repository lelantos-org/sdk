...
export interface EthSigner {
    /// Caller's chain id at the time the signer was constructed. Pinning
    /// this here means downstream code doesn't have to re-ask the RPC each
    /// time it builds an EIP-712 payload.
...
/// Wrap a raw EIP-1193 provider as `EthSigner`. Used by browser builds
/// (MetaMask, Rabby, hardware wallets via WalletConnect, …). All signing
/// and broadcast goes through `provider.request` so popups land in the
/// wallet the user picked at connect-time.
export class Eip1193Signer implements EthSigner {
    ...
    async signTypedData(
        ...
        // payload. viem owns the canonical serialiser for typed data; we
        // route the same `domain`/`types`/`message` shape through.
        const payload = {
...
/// Local-account signer for Node (tests, scripts, relayer). Uses viem's
/// `privateKeyToAccount` for signing + a viem `WalletClient` for broadcast.
...
/// r/s/v components. Useful for tests that compute signatures manually.
