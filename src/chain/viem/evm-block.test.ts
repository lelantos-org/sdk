...
describe("evmBlockNumber", () => {
    ...
    // 25769577, which MASP hashes into the deposit digest. Using the L2 height
    // reverts DigestMismatch on both flushBatch and cancelDeposit.
    ...
    // Ethereum and OP-stack chains omit the field; the EVM reports the block's
    // own height, so the log's blockNumber is correct.
    ...
    // A blank value must not BigInt-parse to 0, which would produce a digest
    // that never matches.
