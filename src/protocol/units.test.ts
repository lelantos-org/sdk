...
import { circuitAmount, tokenAmount } from "../core/brand.js";
...
describe("index-aware conversions", () => {
    ...
    it("rejects a non-positive index rather than dividing by zero", () => {
        expect(() => toTokenUnits(circuitAmount(1n), 1n, { index: 0n })).toThrow(
            InvalidArgumentError,
        );
        expect(() => toCircuitUnits(tokenAmount(1n), 1n, { index: -1n })).toThrow(
            InvalidArgumentError,
        );
    });
