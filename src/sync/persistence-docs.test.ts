// The persistence backends documented on `TreePersistence` and `NullifierPersistence` must work.
// Both state types carry `bigint[]`, which `JSON.stringify` rejects, so these tests exercise the
// documented encode/decode.
...
describe("documented JSON persistence backends", () => {
    ...
    it("the documented TreePersistence round-trips", async () => {
        ...
        // A leaf whose hex digits are all decimal requires the `0x` prefix: `BigInt("10")` is 10,
        // not 16.
