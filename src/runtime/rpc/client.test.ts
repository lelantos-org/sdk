...
/**
 * In-memory worker double. These tests target the correlation logic, and a real
 * spawn is unreliable across Node/browser under `pool: "forks"`.
 */
...
describe("createWorkerRpc", () => {
    ...
    it("carries the error code and call context on a timeout", async () => {
        ...
        const p = c.call("slow", {}, { details: { chunk: 7 } });
        vi.advanceTimersByTime(51);

        const err = await p.catch((e) => e);
        expect(isWalletError(err, "WORKER_TIMEOUT")).toBe(true);
        expect(err.details).toMatchObject({ chunk: 7, timeoutMs: 50 });
...
describe("createWorkerRpc abort", () => {
    ...
    it("stops accepting work once a response could not be deserialised", async () => {
        ...
        // The dropped reply's id is unrecoverable, so the transport must not
        // take new work.
...
describe("worker error wire", () => {
    it("carries a typed error's own fields across the boundary", async () => {
        // `isWalletError(e, code)` is duck-typed on `code`; the fields a
        // subclass adds must also survive so the narrowed type holds.
    ...
    it("drops values structured clone would reject", async () => {
        // A function on the error would make `postMessage` throw and lose the
        // whole reply.
