...
interface WasmLoaderInitCtx {
    isNode: boolean;
    nodePkgUrl: string | null;
}

interface WasmLoaderConfig<M extends WasmModuleBase> {
    /**
    ...
    /** `file://` URL of the `.wasm` binary, read directly on Node. Unused in a browser. */
    nodeWasmUrl: URL;
    /**
     * `file://` URL of the JS module on Node, reported by `getNodePkgUrl()` so wasm-bindgen-rayon
...
interface WasmLoaderHandle<M extends WasmModuleBase> {
...
export function createWasmLoader<M extends WasmModuleBase>(
    ...
    async function init(): Promise<M> {
        ...
        if (injected) {
            ...
            nodePkgUrl = cfg.nodeJsUrl.href;
            mod = await cfg.defaultImport();
            const bytes = new Uint8Array(await readFile(cfg.nodeWasmUrl));
